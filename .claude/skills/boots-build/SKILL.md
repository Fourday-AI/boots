...
A system at `state/systems/<slug>/system.md` with `stage: build`, a `## scope`
...
5. **Build it in its native location, and bake in what the guided run taught.** Read
   ...
   **First, name the artifact for what it does — do not silently reuse the folder
   ...
   - **Point it at the reference notes and tell it to read them at the moment they
     ...
     `state/systems/<slug>/…` path silently breaks the moment someone copies the skill
...
7. **Stage the place for the test examples — proactively, before the user asks.**
   ...
   ```bash
   mkdir -p state/systems/<slug>/inputs
   pwd   # resolve the ABSOLUTE path so you can tell the user how to find the folder
   ```
   Write `state/systems/<slug>/inputs/README.md` — this is the one surface the user
   ...
   > **⌘⇧G**, paste `<abs>/state/systems/<slug>/inputs`, and hit enter (or just ask me
   ...
   Do NOT just hand them the `state/systems/<slug>/inputs/` path with no way to find it
