...
description: >
  ...
  scope, build, review, verify, ship, track, surface, extract, retire). Works in
...
tied to any one repo's idiom: if you find yourself assuming "a system is a Python
feature" just because the current repo has a `toolkit/`, stop and re-read the
palette. The form is chosen from the nature of the work, not the repo you are
standing in.
...
`state/systems/<slug>/system.md`, the next stage reads it. Nothing lives in your
head between stages.
...
read should feel like the last chat never closed: "Last time you'd run the fee-form
tool on 12 accounts and they all checked out, then hit 26 that flagged — you were
working out which ones aren't due yet versus a real data problem. That's where we
are." Then the concrete next move. Only after that, the rest of the board.
...
`state/systems/<slug>/system.md` is the whole record. Shape:
...
Create the folder and files by hand, they are plain markdown. `state/systems/` is
...
`state/systems/<slug>/inputs/` is developer-speak — a nontechnical user in a chat
...
- **Describe how to find it — don't dump a raw path.** Resolve the *absolute* path
  (`pwd` + the folder) and give them the move that turns a path into a place: on
  macOS, "in Finder, press ⌘⇧G, paste this, hit enter" — `<abs>/state/systems/<slug>/inputs`.
  (Offer to open it for them if they'd rather; don't do it unprompted.) Call it "a
