...
- **"Share my progress" — the full picture; recommended.** Description: sends the stage each of your systems reaches, tied to one random ID so the maintainers can see a whole journey and where it broke. → \`${bin}/boots-config set telemetry community\`
- **"Counts only"** — no ID, so they see totals but can't tell one person's run from another. → \`${bin}/boots-config set telemetry anonymous\`
- **"Nothing"** — Boots sends nothing at all. → \`${bin}/boots-config set telemetry off\`

Ask once and take the answer. Do not re-ask, re-frame, or push a second time if they decline — one question, then straight into their work. They can change it later with \`${bin}/boots-config set telemetry <tier>\`; mention that only if they hesitate.
