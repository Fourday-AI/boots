...
description: >
  ...
  of this platform's forms it is — in the place Cowork actually runs it.
...
BOOTS_HOME="<home>" bash "<bin>/boots-telemetry-log" \
  --skill "boots-build" --outcome "OUTCOME" --session-id "<the SESSION_ID Step B printed>" \
  2>/dev/null || true
```
...
A system at `$BOOTS_HOME/systems/<slug>/system.md` with `stage: build`, a `## scope`
...
4. **For a judgment system, do it once by hand first (the guided run).** If the form
   ...
   **What the guided run captures is HOW the user thinks — not what the right answer
   was.** Those are different, and confusing them is the trap here. The corrections
   ("a career gap isn't a negative") are durable and belong in the artifact. The
   *verdict* the run reached on that one example is a draft, produced by you and the
   user under time pressure, and it is exactly the kind of thing verify exists to
   overturn. **Never record the guided run's conclusion as the answer verify should
   expect** — that turns a draft into a fixture and teaches verify to grade the tool
   against your first guess instead of against reality. Record it as "what we thought,
   and why", and let verify disagree.

   **And check the working tree before you treat any recorded claim as fact.** A
   guided run reasons from the system files and whatever the user says, and both go
   stale. If the run leans on something being true — a file exists, a mechanism is
   built, a decision was made — go and look before you build on it. Observed live: a
   guided run took a claim from a project's own notes, built a test case on it, and
   verify found the claim had never been true on disk.
5. **Build it in its native location, and bake in what the guided run taught.** Read
   the palette for this system's platform, `${CLAUDE_PLUGIN_ROOT}/skills/boots/forms/<platform>.md`
   (default `cowork.md`), for where the chosen form lives and how it is
   ...
   **First, name the artifact for what it does — do not silently reuse the folder
   ...
   - **Point it at the reference notes and tell it to read them at the moment they
     ...
     `$BOOTS_HOME/systems/<slug>/…` path silently breaks the moment someone copies the skill
...
6. **Wire any outside connections scope named — you do the setup, the user only
   ...
   **Fall back to a hosted connection only when the marketplace has nothing.** If the
   ...
   register the MCP endpoint where Cowork reads its configuration, for a form the
...
7. **Stage the place for the test examples — proactively, before the user asks.**
   ...
   ```bash
   mkdir -p $BOOTS_HOME/systems/<slug>/inputs
   echo $BOOTS_HOME/systems/<slug>/inputs   # the ABSOLUTE path (~ expands) to tell the user how to find the folder
   ```
   Write `$BOOTS_HOME/systems/<slug>/inputs/README.md` — this is the one surface the user
   ...
   > **⌘⇧G**, paste `$BOOTS_HOME/systems/<slug>/inputs`, and hit enter (or just ask me
   ...
   Do NOT just hand them the `$BOOTS_HOME/systems/<slug>/inputs/` path with no way to find it
...
```bash
<bin>/boots-event --system "<slug>" --event transition --from "<the stage the record was actually at before you moved it>" --to build --outcome advanced 2>/dev/null || true
```
