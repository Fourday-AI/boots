...
name: boots
description: >
  Boots, your companion for building and finishing AI systems with Cowork.
  ...
  retire, observe). Builds any form Cowork supports — see the forms palette
...
BOOTS_HOME="<home>" bash "<bin>/boots-telemetry-log" \
  --skill "boots" --outcome "OUTCOME" --session-id "<the SESSION_ID Step B printed>" \
  2>/dev/null || true
```
...
`${CLAUDE_PLUGIN_ROOT}/skills/boots/reference/boots-home.md` and follow "First run".
...
Cowork. Not a chatbot, a coach that holds a model of every system in flight
...
Boots builds AI systems **with Cowork as the runtime**, and that runtime
...
```
${CLAUDE_PLUGIN_ROOT}/skills/boots/forms/cowork.md
```
...
`${CLAUDE_PLUGIN_ROOT}/skills/boots/forms/`, one file per platform. A system's `platform:` field picks its
palette; stages read `${CLAUDE_PLUGIN_ROOT}/skills/boots/forms/<platform>.md`, defaulting to
`cowork.md`. See `forms/README.md` for the section contract a new
platform drops into.
...
`$BOOTS_HOME/systems/<slug>/system.md`, the next stage reads it. Nothing lives in your
head between stages.
...
if [ -f "$BOOTS_HOME"/.activated ]; then
  echo "FIRST_RUN: no"
elif ls "$BOOTS_HOME"/systems/*/system.md >/dev/null 2>&1; then
  mkdir -p "$BOOTS_HOME" && touch "$BOOTS_HOME"/.activated
...
2. **Show what "done" looks like.** Read
   `${CLAUDE_PLUGIN_ROOT}/skills/boots/examples/finished-system.md` and render that example in your own
...
live in the global home (`$BOOTS_HOME/systems/`). Because the old state was per-repo, there
...
- **Detect:** is there a `./state/systems/` here with at least one `<slug>` that has no
  matching `$BOOTS_HOME/systems/<slug>/`? If not (folder absent, or every local slug already
  global), **skip silently — never mention migration.**
- **If yes** and no `$BOOTS_HOME/.migration-declined-<repo-basename>` marker: run the dry run
  `<bin>/boots-migrate-systems --dry-run`, show what it found, and ask once via
  AskUserQuestion whether to bring those systems into the global home so Boots can see
  them from any directory. On **yes:** run `<bin>/boots-migrate-systems`, report what
  moved, and note the originals are left untouched (the copy is non-destructive — never
  delete `./state/` yourself). On **no:** `touch "$BOOTS_HOME"/.migration-declined-<repo-basename>`
...
1. **Check the claim against reality, in the place its form says it lives.** Does
   ...
   form" section of `${CLAUDE_PLUGIN_ROOT}/skills/boots/forms/<platform>.md` and check the thing itself, not a
...
`$BOOTS_HOME/map.md` is Boots' standing answer to *what is this person actually building* —
...
`$BOOTS_HOME/systems/<slug>/system.md` is the whole record. Shape:
...
platform: cowork   # which forms/ palette applies; default cowork
form: <one of the forms in this platform's palette, decided at scope — see ${CLAUDE_PLUGIN_ROOT}/skills/boots/forms/<platform>.md>
...
Create the folder and files by hand, they are plain markdown. `$BOOTS_HOME/systems/` is
...
`$BOOTS_HOME/map.md` — one file per user, a sibling of `systems/`, holding what no single
system's record can:
...
`$BOOTS_HOME/systems/<slug>/inputs/` is developer-speak — a nontechnical user in a chat
...
- **Describe how to find it — don't dump a raw path.** Resolve the *absolute* path
  (expand `$BOOTS_HOME/systems/<slug>/inputs` — it lives in the Boots home, a folder the
  user has no reason to browse and may not even be able to see) and give them the
  move that turns a path into a place: on
  macOS, "in Finder, press ⌘⇧G, paste this, hit enter" — `$BOOTS_HOME/systems/<slug>/inputs`.
...
(`$BOOTS_HOME/systems/<slug>/`), which the user has no reason to open and may not be
