...
BOOTS_HOME="<home>" bash "<bin>/boots-telemetry-log" \
  --skill "boots-scope" --outcome "OUTCOME" --session-id "<the SESSION_ID Step B printed>" \
  2>/dev/null || true
```
...
A system at `$BOOTS_HOME/systems/<slug>/system.md` with `stage: scope` and a
`## foundation` section.
...
1. **Read the foundation.** Find the one capability that, alone, would already be
   ...
   **Then, before you cut anything, look at what the user already has.** This is the
   last cheap moment — after this, every mistake costs a build. Read `$BOOTS_HOME/map.md`
   if it exists, then the `question:`, `target:`, `form:`, `status:` and `##
   scope` of every other `$BOOTS_HOME/systems/*/system.md`. You are asking four things,
   in this order:
...
3. **Decide the platform and the form. This is the step that was missing.** The
   platform is the runtime the system runs on, default `cowork`; set it
   explicitly only if the user is targeting something else. Read the palette for
   that platform, `${CLAUDE_PLUGIN_ROOT}/skills/boots/forms/<platform>.md` (default
   `cowork.md`). Pick the form from the *nature of the work*, not from what
...
status: scoped
stage: build
platform: <platform, default cowork>
...
```bash
<bin>/boots-event --system "<slug>" --event transition --from "<the stage the record was actually at before you moved it>" --to scope --outcome advanced 2>/dev/null || true
```
